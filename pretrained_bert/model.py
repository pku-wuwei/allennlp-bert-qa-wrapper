...
class BertForQuestionAnswering(Model):
    ...
    def convert_examples_to_features(self, examples):
        ...
        for (example_index, (question_text, paragraph_text, origin_answer_text, start_position, end_position,
            ...
            tok_start_position = None  # 找WordPiece之后的start_index, end_index
            tok_end_position = None
            if self.training and len(origin_answer_text) == 0:  # 没答案就指向[CLS]
                tok_start_position = -1
                tok_end_position = -1
            if self.training and len(origin_answer_text) > 0:  # start指向答案第一个单词的第一个WordPiece，end指向最后一个单词的最后一个WordPiece
    ...
    def forward(self,  # type: ignore
        ...
        if self.training:
            all_start_positions = torch.tensor([f["start_position"] for f in features], dtype=torch.long).cuda()
            all_end_positions = torch.tensor([f["end_position"] for f in features], dtype=torch.long).cuda()
